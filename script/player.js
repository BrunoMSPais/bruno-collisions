class Player {
  ...
  update() {
    ...
    let vX = (document.getElementById("xSpeed").innerText = Math.abs(
      this.velocityX
    ));
    let vY = (document.getElementById("ySpeed").innerText = Math.round(
      Math.abs(this.velocityY) * 10,
      2
    ));
    let altitude = (document.getElementById(
      "playerAltitude"
    ).innerText = Math.floor(this.height - this.y - this.playerHeight));
    let distance = (document.getElementById("distanceToTarget").innerText =
      Math.sqrt(
        (this.y - this.target.y) ** 2 + (this.x - this.target.x - 115) ** 2
      ) - 50);
  }
}
